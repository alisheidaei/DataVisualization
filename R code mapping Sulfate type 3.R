...
DataT <- readRDS("aqdh_pm25component_so4_2019_urban.rds") 
...
ggplot() +
  ...
  labs(title = "", fill = "Sulfate", x = "Longitude", y = "Latitude") +
  theme_minimal() +
...
ggsave("new address/Map SO4 1155 Barroilhet.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "SO4")
write.csv(raster_df, "new address/Data SO4 1155 Barroilhet.csv", row.names = F)
...
# Save the plot as a PDF with the clipped shapefile and raster
ggsave("new address/Map SO4 1054 Mission.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "SO4")
write.csv(raster_df, "new address/Data SO4 1054 Mission.csv", row.names = F)
