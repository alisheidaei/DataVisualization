...
DataT <- readRDS("aqdh_pm25component_ec_2019_urban.rds") 
...
ggplot() +
  ...
  labs(title = "", fill = "Elemental Carbon", x = "Longitude", y = "Latitude") +
  theme_minimal() +
...
ggsave("new address/Map EC 1155 Barroilhet.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "EC")
write.csv(raster_df, "new address/Data EC 1155 Barroilhet.csv", row.names = F)
...
# Save the plot as a PDF with the clipped shapefile and raster
ggsave("new address/Map EC 1054 Mission.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "EC")
write.csv(raster_df, "new address/Data EC 1054 Mission.csv", row.names = F)
