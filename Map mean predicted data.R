...
MapDataMXGBoost <- TotalData %>%
  select(site_id, lat, lon, date, so4_pred_MXGBoost, ca_pred_MXGBoost, 
         si_pred_MXGBoost, ec_pred_MXGBoost) %>%
  filter(complete.cases(.)) %>%
  group_by(site_id, lat, lon) %>%
  summarise(ca = mean(ca_pred_MXGBoost), ec = mean(ec_pred_MXGBoost), 
            si = mean(si_pred_MXGBoost), so4 = mean(so4_pred_MXGBoost)) %>%
  ungroup() 

MapDataMRF <- TotalData %>%
  select(site_id, lat, lon, date, so4_pred_MRF, ca_pred_MRF, 
         si_pred_MRF, ec_pred_MRF) %>%
  filter(complete.cases(.)) %>%
  group_by(site_id, lat, lon) %>%
  summarise(ca = mean(ca_pred_MRF), ec = mean(ec_pred_MRF), 
            si = mean(si_pred_MRF), so4 = mean(so4_pred_MRF)) %>%
  ungroup() 
...
# ggsave("Geographical distribution of components MXGBoost.pdf", width = 12, height = 7, units = "in", scale = 1.8)
ggsave("Geographical distribution of components MRF.pdf", width = 12, height = 7, units = "in", scale = 1.8)
