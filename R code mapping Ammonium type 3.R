...
DataT <- readRDS("aqdh_pm25component_nh4_2019_urban.rds") 
...
ggsave("new address/Map NH4 1155 Barroilhet.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "NH4")
write.csv(raster_df, "new address/Data NH4 1155 Barroilhet.csv", row.names = F)
...
# Save the plot as a PDF with the clipped shapefile and raster
ggsave("new address/Map NH4 1054 Mission.pdf", width = 10, height = 8, dpi = 300)
names(raster_df) <- c("Longitude", "Latitude", "NH4")
write.csv(raster_df, "new address/Data NH4 1054 Mission.csv", row.names = F)
